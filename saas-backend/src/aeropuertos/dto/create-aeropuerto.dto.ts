...
export class CreateAeropuertoDto {
    ...
    @Matches(/^[A-Z]{3}$/, {
        message:
            'El código IATA del aeropuerto debe contener exactamente 3 letras',
    })
    @Transform(({ value }) =>
        typeof value === 'string'
    ...
    @Matches(/^[A-Z]{4}$/, {
        message:
            'El código ICAO del aeropuerto debe contener exactamente 4 letras',
    })
    @Transform(({ value }) =>
        typeof value === 'string'
            ? value.trim().toUpperCase()
    ...
    @Length(3, 150, {
        message:
            'El nombre del aeropuerto debe contener entre 3 y 150 caracteres',
    })
    @Transform(({ value }) =>
    ...
    @Length(2, 100, {
        message:
            'La ciudad del aeropuerto debe contener entre 2 y 100 caracteres',
    })
    @Transform(({ value }) =>
        typeof value === 'string' ? value.trim() : value,
    )
    ...
    @Length(2, 80, {
        message:
            'El país del aeropuerto debe contener entre 2 y 80 caracteres',
    })
    @Transform(({ value }) =>
        typeof value === 'string' ? value.trim() : value,
    )
