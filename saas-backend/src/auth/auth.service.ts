...
export class AuthService {
  ...
  private verificarContrasena(
    ...
    const [saltContrasena, hashContrasena] = partesContrasena;
    ...
    return new Promise((resolve, reject) => {
      scrypt(
        ...
        (error, claveDerivada) => {
          ...
          const hashAlmacenado = Buffer.from(hashContrasena, 'hex');

          if (hashAlmacenado.length !== claveDerivada.length) {
            resolve(false);
            return;
          }

          resolve(timingSafeEqual(hashAlmacenado, claveDerivada));
        },
      );
    });
  }
  ...
  async login(loginDto: LoginDto) {
    ...
    const contrasenaCorrecta = await this.verificarContrasena(
    ...
    if (usuarioEncontrado.rolUsuario !== RolUsuario.SUPERADMIN) {
      ...
      if (
        usuarioEncontrado.aerolineaUsuario.estadoAerolinea !==
      ...
      const suscripcionActiva = await this.prisma.suscripcion.findFirst({
        where: {
          fkAerolineaSuscripcion: usuarioEncontrado.fkAerolineaUsuario,
      ...
      if (
        !suscripcionActiva ||
        suscripcionActiva.planSuscripcion.estadoPlan !== EstadoPlan.ACTIVO
      ) {
        throw new ForbiddenException(
    ...
    const contenidoToken = {
      ...
      fkAerolineaUsuario: usuarioEncontrado.fkAerolineaUsuario,
    };

    const tokenAcceso = await this.jwtService.signAsync(contenidoToken);
    ...
    return {
      ...
      usuario: {
        idUsuario: usuarioEncontrado.idUsuario,
        fkAerolineaUsuario: usuarioEncontrado.fkAerolineaUsuario,
        nombresUsuario: usuarioEncontrado.nombresUsuario,
        apellidosUsuario: usuarioEncontrado.apellidosUsuario,
        correoUsuario: usuarioEncontrado.correoUsuario,
        rolUsuario: usuarioEncontrado.rolUsuario,
        estadoUsuario: usuarioEncontrado.estadoUsuario,
        ultimoAccesoUsuario: fechaUltimoAcceso,
        aerolineaUsuario: usuarioEncontrado.aerolineaUsuario,
      },
    };
  }
}
