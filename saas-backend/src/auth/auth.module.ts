...
function convertirDuracionASegundos(duracion: string): number {
  const duracionNormalizada = duracion.trim().toLowerCase();

  const coincidencia = /^(\d+)([smhd])$/.exec(duracionNormalizada);
...
const expiracionJwt = process.env.JWT_EXPIRES_IN ?? '30d';

if (!secretoJwt) {
  throw new Error('La variable JWT_SECRET no está definida en el archivo .env');
}

const expiracionJwtSegundos = convertirDuracionASegundos(expiracionJwt);
